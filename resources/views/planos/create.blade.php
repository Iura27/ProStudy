...
<div>
    <form class="form-create" action="{{ route('planos.store') }}" method="POST">
        @csrf
        <h1>Novo Plano de Estudo</h1>

         <!-- Campo Nota -->
         <div>
            <label class="label-form">Nota:</label><br>
            <textarea class="form-input" id="nota" name="nota" rows="4"></textarea>
        </div>
        ...
        <div class="dropdown">
            ...
            <div class="dropdown-content" id="horarioDropdown">
                @foreach($horarios as $horario)
                    <label>
                        <input type="checkbox" name="horario_id[]" value="{{ $horario->id }}" onchange="updateSelected('horarioDropdown', 'horarioLabel')">
        ...
        <div class="dropdown">
            ...
            <div class="dropdown-content" id="tarefaDropdown">
                @foreach($tarefas as $tarefa)
                    <label>
                        <input type="checkbox" name="tarefa_id[]" value="{{ $tarefa->id }}" onchange="updateSelected('tarefaDropdown', 'tarefaLabel')">
                        {{ $tarefa->descricao }}
                    </label>
                @endforeach
            </div>
